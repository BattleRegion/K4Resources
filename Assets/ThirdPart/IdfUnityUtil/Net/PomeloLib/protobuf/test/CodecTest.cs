...
namespace Pomelo.Protobuf.Test
{
	public class CodecTest
	{
		...
		public static bool Run(){
            ...
            //Debug.Log("Encode sint32 test finished , result is : {1}, cost time : {0}", end - start, flag);
			if(!flag) success = false;

			start = DateTime.Now;
			flag = EncodeUInt32Test(10000);
			end = DateTime.Now;
            //Debug.Log("Encode uint32 test finished , result is : {1}, cost time : {0}", end - start, flag);
			if(!flag) success = false;
